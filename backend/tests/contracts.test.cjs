...
test('hash contratual é estável e PDF contém um documento válido', async () => {
  const snapshot = {
    ...
    plan: { code: 'START', name: 'SKS Start', setupAmountCents: 59000, monthlyAmountCents: 19700, implementationItems: ['Configuração inicial'], features: ['Etiquetas'] },
    acceptedAt: '2026-07-25T12:00:00.000Z', implementationDays: 15, version: '2026-07-25',
  };
  assert.equal(contractHash(snapshot), contractHash({ ...snapshot }));
  const pdf = await renderContractPdf({
    id: 'contract-1', restaurantId: 'restaurant-1', implementationOrderId: 'order-1', acceptedById: 'user-1',
    ...
    welcomeEmailedAt: null, welcomeEmailProviderId: null, welcomeEmailError: null,
